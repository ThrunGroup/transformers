...
def inference_pipeline(model_names: List[str] = ["bert-base-cased"], num_experiments: int = 1):
    ...
    sentiment_analysis_pipelines = {
        model_name: pipeline("sentiment-analysis", model=model, tokenizer=AutoTokenizer.from_pretrained(model_name))
    ...
    for model_name, sentiment_analysis_pipeline in sentiment_analysis_pipelines.items():
